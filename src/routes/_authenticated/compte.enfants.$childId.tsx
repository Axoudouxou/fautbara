...
import { ArrowLeft, BookOpen, CalendarClock, CalendarPlus, ClipboardList, Loader2, Search, UserRound } from "lucide-react";
  ...
  const journeyQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      const [packs, bookings, reports, conversations] = await Promise.all([
        ...
        supabase.from("session_reports").select("id, booking_id, teacher_id, attendance, content_note, progress_level, homework_done, engagement_rating, next_steps, created_at").eq("learner_id", user.id).eq("child_id", childId).order("created_at", { ascending: false }).limit(4),
  ...
  return (
    <main className="container-page py-8 sm:py-12">
      <Link to="/compte/enfants" className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground"><ArrowLeft className="size-4" /> Mes enfants</Link>
       <div className="mt-5 flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-4"><UserAvatar name={data.child.first_name} className="size-16" /><div><p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">Parcours enfant</p><h1 className="font-display text-3xl font-bold text-foreground">{data.child.first_name}</h1><p className="text-sm text-muted-foreground">{data.child.school_level || "Niveau à préciser"}</p></div></div><Link to="/professeurs" search={{ enfant: childId }} className="inline-flex items-center gap-2 rounded-xl bg-primary px-4 py-3 text-sm font-semibold text-primary-foreground"><Search className="size-4" /> Trouver un intervenant</Link></div>
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-[1.25fr_0.75fr]">
       ...
       <section className="mt-6"><div className="flex items-center justify-between gap-3"><h2 className="font-display text-xl font-bold text-foreground">Formules actives</h2><Link to="/compte/reservations" search={{ enfant: childId }} className="text-sm font-semibold text-primary hover:underline">Gérer les cours</Link></div>{activePacks.length ? <ul className="mt-4 grid gap-4 md:grid-cols-2">{activePacks.map((pack) => { const left = Math.max(pack.sessions_total - pack.sessions_used, 0); const teacher = profileMap.get(pack.teacher_id); return <li key={pack.id} className="rounded-3xl border border-border bg-card p-6 shadow-[var(--shadow-card)]"><p className="text-xs font-bold uppercase tracking-wide text-primary">{pack.teacher_offers?.subjects?.name}</p><h3 className="mt-1 font-display text-lg font-bold text-foreground">Formule {pack.pack_types?.name}</h3><div className="mt-3 flex items-center gap-3"><UserAvatar name={teacher?.display_name ?? "Intervenant"} src={teacher?.avatar_url} className="size-10" /><span className="text-sm font-semibold text-foreground">{teacher?.display_name ?? "Intervenant"}</span></div><div className="mt-5"><ProgressBar value={(pack.sessions_used / pack.sessions_total) * 100} label={`${left} séance${left > 1 ? "s" : ""} restante${left > 1 ? "s" : ""}`} /></div><p className="mt-3 text-xs text-muted-foreground">Cette formule est valable jusqu’au {formatDate(pack.expires_at)}.</p>{left > 0 && <Link to="/compte/programmer/$packId" params={{ packId: pack.id }} className="mt-4 inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-xs font-semibold text-primary-foreground"><CalendarPlus className="size-3.5" /> Programmer une séance</Link>}</li>; })}</ul> : <div className="mt-4"><EmptyState icon={BookOpen} title="Aucune formule active" description={`Les formules de ${data.child.first_name} apparaîtront ici après leur achat.`} action={<Link to="/professeurs" search={{ enfant: childId }} className="inline-flex rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground">Trouver un intervenant</Link>} /></div>}</section>

       <section className="mt-8 rounded-3xl border border-border bg-card p-6 shadow-[var(--shadow-card)]"><h2 className="font-display text-xl font-bold text-foreground">Derniers comptes-rendus</h2>{data.reports.length ? <ul className="mt-4 space-y-4">{data.reports.map((report) => <li key={report.id} className="rounded-2xl bg-secondary/50 p-4"><p className="text-xs font-semibold text-muted-foreground">{formatDate(report.created_at)}</p><dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-xs text-muted-foreground">Présence</dt><dd className="font-semibold text-foreground">{report.attendance}</dd></div><div><dt className="text-xs text-muted-foreground">Niveau d’avancement</dt><dd className="font-semibold text-foreground">{report.progress_level}</dd></div><div className="sm:col-span-2"><dt className="text-xs text-muted-foreground">Contenu travaillé</dt><dd className="text-foreground">{report.content_note}</dd></div><div><dt className="text-xs text-muted-foreground">Travail fait depuis la dernière fois</dt><dd className="text-foreground">{report.homework_done || "Non renseigné"}</dd></div><div><dt className="text-xs text-muted-foreground">Engagement</dt><dd className="text-foreground">{report.engagement_rating}/5</dd></div><div className="sm:col-span-2"><dt className="text-xs text-muted-foreground">Note pour la prochaine fois</dt><dd className="text-foreground">{report.next_steps || "Non renseignée"}</dd></div></dl></li>)}</ul> : <p className="mt-3 text-sm text-muted-foreground">Les retours des intervenants apparaîtront après les séances réalisées.</p>}</section>
    </main>
  );
}