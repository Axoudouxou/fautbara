-- Portefeuille électronique (wallet) par compte, façon TIKERAMA : remplace
-- le crédit spécifique à un professeur (booking_reschedule_credits) par un
-- solde général utilisable sur n'importe quelle réservation, et redirige
-- tous les remboursements (annulation, no-show professeur, report tardif du
-- professeur, remboursement exceptionnel) vers ce solde plutôt que de se
-- contenter d'une mise à jour de statut sans mouvement d'argent réel — ce
-- qu'ils faisaient jusqu'ici (le formulaire d'annulation l'affichait même
-- explicitement : "aucun mouvement d'argent réel n'est effectué").
...
-- Ni insert ni update en direct : tout passe par request_wallet_withdrawal
-- et admin_process_wallet_withdrawal (SECURITY DEFINER), pour que le débit
-- du solde et la validation admin restent sous contrôle serveur.
...
-- =========================================================
-- Primitives de crédit/débit (internes : jamais exposées en RPC direct,
-- seulement appelées par d'autres fonctions SECURITY DEFINER ci-dessous —
-- le propriétaire des fonctions a toujours implicitement le droit
-- d'exécuter ses propres fonctions, REVOKE ALL n'empêche que les appels
-- directs via PostgREST/RPC).
-- =========================================================
...
-- =========================================================
-- Retrait vers Mobile Money : le solde est débité dès la demande (évite un
-- double retrait pendant qu'une première demande est encore en attente),
-- et recrédité si l'admin la refuse.
-- =========================================================
...
-- =========================================================
-- Migration de l'ancien système de crédit spécifique à un professeur vers
-- le portefeuille général, puis suppression : plus rien ne doit s'appuyer
-- dessus après cette migration.
-- =========================================================
...
-- balance_after est approximatif ici (calculé sur le solde final du
-- portefeuille, pas un vrai solde courant ligne par ligne) : c'est un
-- transfert ponctuel d'historique, seul wallets.balance_fcfa fait foi.
...
-- =========================================================
-- payments.wallet_used_fcfa : part du prix réglée depuis le portefeuille,
-- distincte de amount_fcfa (ce que Jèko collecte réellement). La commission
-- et la part professeur se calculent désormais sur le prix plein, jamais
-- sur le seul reliquat Jèko — sinon un paiement entièrement couvert par le
-- portefeuille ferait travailler le professeur gratuitement.
-- =========================================================
...
begin
  ...
  -- Idempotent : un rappel (ex. jeko-create-payment après un premier appel
  -- client avec le montant wallet choisi) renvoie la ligne déjà calculée,
  -- sans jamais redébiter le portefeuille une seconde fois.
  ...
  -- Le montant demandé par le client n'est qu'une suggestion : plafonné au
  -- solde réel et au prix de la séance, jamais fait confiance tel quel.
  ...
  if v_amount_due = 0 then
    -- Entièrement réglé par le portefeuille : rien à encaisser via Jèko,
    -- la réservation est confirmée immédiatement (no-op si elle était déjà
    -- "accepted", ex. un paiement de report).
...
-- =========================================================
-- Tous les remboursements passent désormais par le portefeuille plutôt que
-- par une simple mise à jour de statut sans mouvement d'argent réel.
-- =========================================================
...
begin
  ...
  -- Montant réellement en jeu : ce qu'a payé le demandeur, portefeuille
  -- compris, pas seulement la part collectée par Jèko.
...
begin
  ...
  if v_fee_amount > 0 then
    if v_by_parent then
      ...
      -- Report tardif à l'initiative du professeur : le parent est
      -- dédommagé sur son portefeuille général plutôt qu'un crédit
      -- utilisable uniquement avec ce professeur.
      perform public.credit_wallet(
...
-- Liste admin des demandes de retrait avec le nom du demandeur : PostgREST
-- ne peut pas faire ce lien tout seul (wallet_withdrawal_requests et
-- profiles référencent tous deux auth.users, mais pas l'un l'autre), d'où
-- une fonction dédiée qui fait la jointure côté serveur, comme
-- admin_list_teachers pour un besoin similaire.
